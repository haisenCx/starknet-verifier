    ...
    function registerGpsFacts(
        ...
        for (task = 0; task < nTasks; task++) {
            ...
            for (uint256 treePair = 0; treePair < nTreePairs; treePair++) {
                ...
                uint256 nPages = taskMetadataCopy[taskMetadataOffset + METADATA_TASK_HEADER_SIZE + 2 * treePair
                ...
                for (uint256 i = 0; i < nPages; i++) {
                    (uint256 pageSize, uint256 pageHash) =
                        pushPageToStack(pageInfoPtr, curAddr, curOffset, nodeStack, nodeStackLen);
                ...
                // stack.
                uint256 nNodes = taskMetadataCopy[taskMetadataOffset + METADATA_TASK_HEADER_SIZE + 2 * treePair
            ...
            uint256 programHash = taskMetadataCopy[taskMetadataOffset + METADATA_OFFSET_TASK_PROGRAM_HASH];
            ...
            {
                uint256 outputSize = taskMetadataCopy[taskMetadataOffset + METADATA_OFFSET_TASK_OUTPUT_SIZE];
