...
contract GpsStatementVerifier is GpsOutputParser, Identity, CairoBootloaderProgramSize, PrimeFieldElement0 {
    ...
    */
    function registerPublicMemoryMainPage(
    ...
    ) private returns (uint256 publicMemoryLength, uint256 memoryHash, uint256 prod) {
        ...
        publicMemoryLength = (
            PROGRAM_SIZE
            // return fp and pc =
            + 2 + N_MAIN_ARGS + N_MAIN_RETURN_VALUES
        ...
        {
            {
                ...
                for (uint256 task = 0; task < nTasks; task++) {
                    ...
                    // (the bound is somewhat arbitrary).
                    require(
                        1 <= nTreePairs && nTreePairs < 2 ** 20, "Invalid number of pairs in the Merkle tree structure."
                    );
        ...
        bytes32 factHash;
        (factHash, memoryHash, prod) =
            memoryPageFactRegistry.registerRegularMemoryPage(publicMemory, z, alpha, K_MODULUS);
    }
}
